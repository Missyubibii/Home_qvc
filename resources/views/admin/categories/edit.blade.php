...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Chỉnh sửa danh mục</h1>
            <p class="text-sm text-gray-500 mt-1">Cập nhật thông tin cho danh mục: {{ $category->name }}</p>
        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="slug" class="block font-medium text-sm text-gray-700">Slug (Đường
                                    dẫn)</label>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="6"
                                class="block w-full pl-4 pr-10 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">{{ old('description', $category->description) }}</textarea>
                        </div>
                            ...
                            <div class="relative flex items-start">
                                <div class="flex h-5 items-center">
                                    <input id="show_on_menu" name="show_on_menu" type="checkbox" value="1"
                                        @checked(old('show_on_menu', $category->show_on_menu))
                                        class="h-4 w-4 rounded border-gray-500 text-red-600 focus:ring-red-500">
                                </div>
                                <div class="ml-3 text-sm">
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        {{-- Ảnh danh mục --}}
                            <label class=" font-medium text-sm text-gray-700 mb-2">Ảnh danh mục</label>
                            <div class="w-full h-48 bg-gray-100 rounded-md flex items-center justify-center border-2 border-dashed border-gray-300">
                                {{-- SỬA Ở ĐÂY: Thay đổi đường dẫn ảnh --}}
                                <img id="image-preview" src="{{ $category->image ? asset('media/category/' . $category->image) : '' }}" alt="Xem trước ảnh" class="{{ $category->image ? '' : 'hidden' }} w-full h-full object-cover rounded-md">
                                <span id="image-placeholder" class="{{ $category->image ? 'hidden' : '' }} text-gray-500">Xem trước ảnh</span>
                            </div>
                            <input type="file" name="image" id="image" class="hidden" accept="image/*">
                            <button type="button" onclick="document.getElementById('image').click()" class="mt-2 w-full text-sm text-red-600 bg-red-50 hover:bg-red-100 rounded-md py-2">
                                Tải ảnh lên
                            </button>
                            <p class="mt-1 text-xs text-gray-500 text-center">Để trống nếu không muốn thay đổi.</p>
